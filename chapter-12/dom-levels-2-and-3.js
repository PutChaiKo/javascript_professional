    ...
    // 12.2 样式
        ...
        // 12.2.3 元素大小
            ...
            // 4.确定元素大小
                ...
                {
                    if (typeof arguments.callee.offset != "number") // arguments.callee 指向当前函数的引用
                    {
                        var scrollTop = document.documentElement.scrollTop; // 被卷走的页面高度
                        var temp = document.createElement("div");   // 新建一个 div
                        temp.style.cssText = "position:absolute;left:0;top:0;"; // 赋予 div CSS
                        document.body.appendChild(temp);    // 将 div 放置到 body 中
                        arguments.callee.offset = -temp.getBoundingClientRect().top - scrollTop;
                        // 上行代码中，将 element 的 offset 设置为 -0 或 -2
                        // 原理为 getBoundingClientRect().top 是元素相对于视窗的位置，如果被上滚即为负数
                        // document.documentElement.scrollTop 获得页面被滚掉的页面长度
                        // 如果是 IE8 之前，在原点的元素 getBoundingClientRect().top 会奇葩地返回 2
                        // 所以 offset 将设置为 -2
                ...
                function getBoundingClientRect(element)
                {
                    ...
                    else
                    {
                        ...
                        return {
                            left: actual - scrollLeft,
                            right: actual + element.offsetWidth - scrollLeft,
        ...
        // 12.3 遍历
            // 12.3.1 NodeIterator 类型
                // 可使用 document.createNodeIterator() 方法创建新实例，接受四个参数
                // root 搜索起点
                // whatToShow 哪些节点，是一个位掩码
                // filter NodeFilter 对象，或者一个函数
                // entityReferenceExpansion 布尔值，表示是否要扩展实体引用。在 HTML 没用。
                ...
                // filter 也可以是与 acceotNode() 方法类似的函数
